...
from superset.commands.importers.v1.amararaja import ImportAmaraRajaCommand
...
def load_amararaja_from_configs(
    force_data: bool = False, load_test_data: bool = False
) -> None:
    """
    Load all the examples inside superset/examples/configs/.
    """
    contents = load_amararaja_contents(load_test_data)
    command = ImportAmaraRajaCommand(contents, overwrite=True, force_data=force_data)
    command.run()
